...
echo "Is $HOME/.steam/steam/steamapps/compatdata/284160/pfx/drive_c/users/$USER/AppData/Local/BeamNG.drive/0.32/mods/ is this the path of BeamNG.Drive's mods folder for proton? [y/n]"

if ! yes_or_no; then
    read -p "Please enter BeamNG.Drive's mods folder for proton: " protonpath
else
    protonpath="$HOME/.steam/steam/steamapps/compatdata/284160/pfx/drive_c/users/$USER/AppData/Local/BeamNG.drive/0.32/mods"
fi

echo "Is $HOME/.local/share/BeamNG.drive/0.32/mods/ is this the path of BeamNG.Drive's mods folder for native? [y/n]"

if ! yes_or_no; then
    read -p "Please enter BeamNG.Drive's mods folder for proton: " nativepath
else
    nativepath="$HOME/.local/share/BeamNG.drive/0.32/mods"
fi
...
cat launch-beammp-linux-template.sh | sed "s:PMODS:$protonpath:g" | sed "s:NMODS:${nativepath}/multiplayer:g" | sed "s:MPID:$beammpid:g" | sed "s:NATID:$beamngid:g" > beammp-linux.sh
